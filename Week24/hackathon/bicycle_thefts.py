...
def main():
    ...
    for item in json_data:
        ...
        item['REPORT_DATE'] = datetime.utcfromtimestamp(item['REPORT_DATE'] / 1000.0)
    ...
    df['OCC_DATE'].groupby(df['OCC_MONTH']).count().plot(kind='bar', color='skyblue')
    ...
    plt.title('Distribution of Incidents by Weekday')
    plt.xlabel('Weekday')
    plt.ylabel('Number of Incidents')
    plt.grid(True)
    plt.show()
